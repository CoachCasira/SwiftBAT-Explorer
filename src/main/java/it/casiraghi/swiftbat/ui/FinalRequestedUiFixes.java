...
/**
 * Final deterministic pass for skin-dependent visual details. It runs after the
 * older enhancers, so table/catalog scroll thumbs cannot collapse to a dot and
 * the Included GRBs export action is never wrapped by a full-width framed strip.
 */
public final class FinalRequestedUiFixes {
    private static final String WATCHED = FinalRequestedUiFixes.class.getName() + ".watched";
    private static final String BAR_DONE = FinalRequestedUiFixes.class.getName() + ".barDone";
    private static final String BAR_SYNC = FinalRequestedUiFixes.class.getName() + ".barSync";
    private static final String INTERNAL_WATCHED = FinalRequestedUiFixes.class.getName() + ".internalWatched";
    private static final String INCLUDED_DONE = FinalRequestedUiFixes.class.getName() + ".includedDone";
    ...
    private static void polishScrollBar(ScrollBar bar) {
        ...
        try {
            ...
            if (thumb instanceof javafx.scene.layout.Region region) {
                double target = targetThumbPixels(bar);
                region.setMinHeight(target);
