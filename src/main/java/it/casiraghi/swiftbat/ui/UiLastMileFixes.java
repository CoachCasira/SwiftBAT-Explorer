...
/**
 * Last-mile JavaFX layout fixes kept deliberately in Java rather than in CSS.
 *
 * <p>This class owns dynamic geometry that previously had multiple competing
 * listeners: application chrome sizing, Population result sizing and vertical
 * TableView scrolling. CSS remains responsible only for colours and visual skin.</p>
 */
public final class UiLastMileFixes {
    ...
    // Prevent FinalUiStabilityEnhancer from installing a second Population
    // geometry controller. We keep its translation/catalog/table safety passes.
    private static final String FINAL_HISTOGRAM_DONE =
            FinalUiStabilityEnhancer.class.getName() + ".histogramDone";
    private static final String FINAL_FILTER_DONE =
            FinalUiStabilityEnhancer.class.getName() + ".filterDone";
    private static final String FINAL_POP_TABS_DONE =
            FinalUiStabilityEnhancer.class.getName() + ".populationTabsDone";
    ...
    /** Call before FinalUiStabilityEnhancer.install(...). */
    public static void prepare(Parent root) {
        if (root == null) return;
        prepareNode(root);
    }

    /** Call after all the other UI enhancers. */
    ...
    private static void applyChrome(Parent root) {
        ...
        if (brand != null) {
            double brandWidth = veryWide ? 252 : roomy ? 232 : 205;
            brand.setMinWidth(brandWidth);
            brand.setPrefWidth(brandWidth);
            brand.setMaxWidth(brandWidth);
        }
        ...
        for (Node child : bar.getChildren()) {
            if (child instanceof Button button && button.getStyleClass().contains("top-nav-button")) {
        ...
        for (int i = 0; i < navButtons.size(); i++) {
            ...
            double[] source = veryWide ? wideWidths : roomy ? roomyWidths : compact;
            double buttonWidth = source[Math.min(i, source.length - 1)];
    ...
    /* ---------------- Population: one geometry controller ---------------- */
    ...
    private static void installPopulationFilterListener(Control control, Parent root) {
        if (Boolean.TRUE.equals(control.getProperties().get(POP_FILTER))) return;
        control.getProperties().put(POP_FILTER, Boolean.TRUE);
        control.visibleProperty().addListener((obs, oldValue, newValue) -> requestPopulation(root));
        control.managedProperty().addListener((obs, oldValue, newValue) -> requestPopulation(root));
    }
    ...
    /* ---------------- All TableViews: scrollbar lives outside the table ---------------- */
    ...
    private static void installExternalTableScrollbar(TableView<?> table) {
        ...
        // Normalize the Population export before reparenting the TableView.
    ...
    private static boolean replaceTableInParent(TableView<?> table, HBox shell) {
        ...
        if (parent instanceof GridPane grid) {
            ...
            Pos alignment = GridPane.getHalignment(table) == null && GridPane.getValignment(table) == null ? null : null;
    ...
    private static boolean wireScrollbar(TableView<?> table, ScrollBar external) {
        ...
        if (previous instanceof ScrollBar oldBar) {
            try {
                ...
                // Skin replacement can invalidate a property between pulses.
            }
        }
    ...
    /* ---------------- Population Included GRBs export ---------------- */
