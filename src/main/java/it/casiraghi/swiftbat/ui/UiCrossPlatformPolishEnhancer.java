...
/**
 * Last cross-platform UI pass for controls whose native skins vary noticeably
 * between macOS and Windows. It also owns the narrow Explorer overview layout.
 */
public final class UiCrossPlatformPolishEnhancer {
    private static final String WATCHED = UiCrossPlatformPolishEnhancer.class.getName() + ".watched";
    private static final String LOCK_DONE = UiCrossPlatformPolishEnhancer.class.getName() + ".lockDone";
    private static final String LOCK_TEXT_SYNC = UiCrossPlatformPolishEnhancer.class.getName() + ".lockTextSync";
    private static final String SPECTRO_DONE = UiCrossPlatformPolishEnhancer.class.getName() + ".spectroDone";
    private static final String OVERVIEW_DONE = UiCrossPlatformPolishEnhancer.class.getName() + ".overviewDone";
    private static final String OVERVIEW_RIGHT = UiCrossPlatformPolishEnhancer.class.getName() + ".overviewRight";
    ...
    private UiCrossPlatformPolishEnhancer() { }
    ...
    private static void enhance(Node node) {
        ...
        if (node instanceof VBox box && box.getStyleClass().contains("overview-chart-card")) {
            installExplorerOverviewResponsive(box);
        }
    }

    /* ---------------- Stable vector lock icon ---------------- */
    ...
    private static void refreshLockGraphic(ToggleButton toggle) {
        ...
        icon.setStyle(toggle.isSelected()
                ? "-fx-fill: #ffffff;"
                : "-fx-fill: #9fc5e8;");
    ...
    /* ---------------- Spectroscopy exact bilingual copy ---------------- */
    ...
    private static void installExplorerOverviewResponsive(VBox chartCard) {
        ...
        if (!Boolean.TRUE.equals(pane.getProperties().get(OVERVIEW_DONE))) {
            ...
            pane.getProperties().put(OVERVIEW_RIGHT, right);
            pane.widthProperty().addListener((obs, oldWidth, newWidth) ->
                    Platform.runLater(() -> applyExplorerOverviewLayout(pane, chartCard)));
            pane.sceneProperty().addListener((obs, oldScene, newScene) -> {
                if (newScene != null) Platform.runLater(() -> applyExplorerOverviewLayout(pane, chartCard));
            });
        }
    ...
    private static void applyExplorerOverviewLayout(BorderPane pane, VBox chartCard) {
        ...
        if (!(stored instanceof Node right)) return;
        double width = pane.getWidth();
        boolean compact = width > 0 && width < 1120;

        chartCard.setMinWidth(0);
        chartCard.setMaxWidth(Double.MAX_VALUE);
        if (compact) {
            if (pane.getRight() == right) pane.setRight(null);
            if (pane.getBottom() != right) pane.setBottom(right);
            BorderPane.setMargin(right, new Insets(12, 0, 0, 0));
            if (right instanceof Region region) {
                region.setMinWidth(0);
                region.setPrefWidth(Region.USE_COMPUTED_SIZE);
                region.setMaxWidth(Double.MAX_VALUE);
            }
            if (right instanceof VBox box) {
                for (Node child : box.getChildren()) {
            ...
            if (pane.getBottom() == right) pane.setBottom(null);
            if (pane.getRight() != right) pane.setRight(right);
            BorderPane.setMargin(right, new Insets(0, 0, 0, 12));
            if (right instanceof Region region) {
                region.setMinWidth(260);
                region.setPrefWidth(300);
                region.setMaxWidth(330);
            }
        }
        pane.requestLayout();
    }
